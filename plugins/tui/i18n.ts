...
export const STRINGS = {
  ...
  "guard.adr.overrideUnknownWarning": { en: "[adr-guard] Unknown adr.{key} value `{value}` — falling back to default. See ADR config schema in `plugins/adr-guard/adr-guard-config.ts`.", "zh-CN": "[adr-guard] 未知的 adr.{key} 值 `{value}` —— 回退为默认值。详见 `plugins/adr-guard/adr-guard-config.ts` 中的 ADR 配置 schema。" },
