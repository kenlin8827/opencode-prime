 ...
 * for Ctrl+A / Ctrl+S?". The OCP dashboard binds install to Ctrl+A
 * (install/src/ui/app.tsx), but terminals encode that chord differently
 * (legacy 0x01, kitty disambiguated CSI 97;5u, kitty alternate-keys
 * CSI 27;5;97u) and some (tmux/screen with prefix C-a, or a terminal
 * binding that swallows Ctrl+A) never deliver it to the app at all.
 *
 * Run INSIDE the terminal where the shortcut misbehaves, from the repo root:
 *
 *   bun scripts/probe-keys.ts
 *
 * Then press Ctrl+A, then Ctrl+S. Each press is echoed as the exact key
 * event the TUI receives. `q` quits.
 *
 * Reading the result:
 *  - a row with  name="a" ctrl=true            → binding fires; app is fine
 ...
 *  - NO row at all for Ctrl+A                 → the terminal/tmux prefix is
...
const lines: string[] = []
const probe = new TextRenderable(renderer, { id: 'probe', content: 'Press Ctrl+A, then Ctrl+S… (q quits)' })
renderer.root.add(probe)

const render = () => {
  probe.content = ['Press Ctrl+A, then Ctrl+S… (q quits)', ...lines].join('\n')
}
