...
三条线性开发流（`/dev-quick`、`/dev-plan`、`/dev-review`）已实现为同一个 [`/dev`](dev.md) 组合引擎的**预设**——每条都展开为 `/dev` 上的一组标志。`/dev-prud` 与 `/dev-ultra` 仍是独立协议（拓扑不同）。无预设匹配时可直接调用 `/dev`。五档之上还有上游协议：[`/dev-deep`](dev-deep.md)——需求模糊到写不出验收标准时先做六段挖掘，产出澄清后的目标再交给合适的开发流（或 `/goal` 契约）。
...
- **断点续跑**: `--resume[=<slug>]` 读取按任务隔离的 `.ocp/dev-ultra/<slug>.md`（支持并行多目标；旧单文件在首次续跑时自动迁移）

---

## 开发流选择决策树

进树之前：若需求模糊到写不出验收标准（代码或分析皆然），先跑 [`/dev-deep`](dev-deep.md)——挖掘产出澄清目标＋约束＋验收标准，再回到本决策树。
