...
| **`/dev-deep <requirement> [--auto] [--retry=N] [--budget=N] [--resume]`** | 开发流 | **Deep-Dev 挖掘先行深度任务闭环**：面向模糊到写不出验收标准的需求（代码或分析皆可）—— 六段需求挖掘 → 门控任务树规划 → 逐节点执行（重试/重规划硬刹车）→ 交付附审计轨迹与恢复卡；`--resume` 从 `.ocp/dev-deep-state.md` 断点续跑（详见[深度任务协议](dev-deep.md)）。独立协议，**不是** `/dev` 预设 |
