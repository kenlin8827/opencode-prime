...
| **`/build-product <一句话需求> [--max-rounds=N] [--parallel] [--no-security] [--no-qc] [--no-gate] [--auto-advisor[=full\|lite\|off]]`** | 开发流 | **Build-Product 一句话流水线**：以产品形态模式调用 `@ultra`（全域主 agent）—— 一句话走完整管线（P1 SPEC → P3 设计 → P4 架构 → P7 实现 → P9 敌对 QC → P10 门禁 → P11 交付）。技术栈无关：栈知识在 `references/constitution/language_stacks.yaml`（开放注册表）。非产品任务（SQL 优化、bug 修复、refactor、脚本）**直接调用 `@ultra`**——它跑轻量的聚焦循环（详见 [Build-Product](build-product.md)） |
