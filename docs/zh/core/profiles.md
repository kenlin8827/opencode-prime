...
> **国际化 (i18n)：** 所有 TUI 向导插件（`/profile`、`/provider`、`/project`）均支持国际化。首次使用时自动从系统区域设置/环境变量检测语言，之后存储在 `~/.config/opencode/ocp.json` 中。每个向导的主菜单底部都有 `🌐 <当前语言>` 选项（8 种语言），可即时切换语言 — 菜单会立即以新语言重新渲染。翻译集中维护在 `plugins/tui/i18n.ts`，按语言分文件存于 `plugins/tui/i18n/locales/`。
