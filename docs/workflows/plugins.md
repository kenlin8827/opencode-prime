...
Because a file can fail to ship two different ways, `/memory status` asks git two questions and warns on both. It first checks whether `public.md` is **tracked** (`git ls-files`); if it is not, it then checks whether an ignore rule matches it. An **ignored** file (a repo whose `.gitignore` excludes the whole `.ocp/` tree is the usual cause) can never ship — the message spells out the `.gitignore` shape that fixes it. An **untracked** file passes every ignore check and still stays local, because nothing has staged it — the message names the `git add` that ships it. A **tracked** file is silent, and any undeterminable verdict (no git, not a repo, subprocess timeout) stays silent rather than crying wolf.
