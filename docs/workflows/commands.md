...
| **`/build-product <one-line requirement> [--max-rounds=N] [--parallel] [--no-security] [--no-qc] [--no-gate] [--auto-advisor[=full\|lite\|off]]`** | Dev Flow | **Build-Product One-Line Pipeline**: invokes `@ultra` (the universal commander) in its product-shaped mode — full pipeline from one sentence (P1 SPEC → P3 design → P4 architecture → P7 implement → P9 adversarial QC → P10 gate → P11 deliver). Tech-stack agnostic: stack knowledge lives in `references/constitution/language_stacks.yaml` (open registry). For non-product tasks (SQL optimization, bug fix, refactor, scripts), invoke `@ultra` directly — it runs a lightweight focused loop (see [Build-Product](build-product.md)) |
