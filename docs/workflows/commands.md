...
| **`/dev-deep <requirement> [--auto] [--retry=N] [--budget=N] [--resume]`** | Dev Flow | **Deep-Dev Mine-First Deep-Task Track**: for requirements too vague to write acceptance criteria (code OR analysis) — 6-part requirement mining → gated task-tree planning → node-wise execution with fixed retry/replan brakes → delivery with audit trail & recovery card; `--resume[=<slug>]` continues from per-task checkpoints `.ocp/dev-deep/<slug>.md` — parallel-run safe (see [Deep-Task Protocol](dev-deep.md)). Standalone protocol, NOT a `/dev` preset |
...
| **`/clean-dead-code [changes\|commit <ref>\|<base>..<head>\|pr <n\|url>\|all] [--focus=..] [--cats=..] [--paths=..] [--exclude=..] [--report-only] [--approve=R1,R2]`** | Quality Loop | Evidence-graded dead-code sweep & cleanup — scans uncommitted changes, a commit/PR/range, or the whole project; `--focus "payment checkout"` (or trailing free text) narrows findings to one feature's code area. Natural-language arguments are fine — prose intent (scope, focus, even flags) is normalized and confirmed once before anything runs. Covers unused declarations, unreachable code, orphan files, unused deps, dead config, orphan resources. Report first; the user approves every deletion and every uncertain finding. Deletions land as small verifiable, individually revertible batches |
...
| **`/project` (or `init`/`index`/`sync` subcommand)** | Project Setup | `init\|index\|sync` subcommands scaffold project baseline files (`.ocp/ocp.json` etc.) and trigger CodeGraph / GitNexus indexing. Bare `/project` opens the interactive wizard menu (toggles MCP services & plugins via visual terminal UI) |
| **`/memory [note\|status\|on\|off]`** (+ `/memory-summarize [focus]`) | Project Memory | Project-level memory in two scopes: `/memory note "<lesson>"` writes to `.ocp/memory/public.md` (committed, PR-reviewed); `--private` writes to `private.md` (gitignored, current-user-only). The LLM can also call the `memory_note` tool when it discovers a reusable rule — that tool defaults to `private` (ADR-2.0.2#01), so team-shared entries need an explicit `scope: "public"`. `/memory status` also warns when `public.md` would not reach the team — both when it is git-ignored (can never ship) and when it is merely untracked (passes every ignore check, still local until someone stages it). `/memory-summarize [focus]` reviews the current session and writes durable lessons. Both files inject under `[PROJECT MEMORY]` (AGENTS.md authoritative). 16k-char cap per section → pointer block. |
