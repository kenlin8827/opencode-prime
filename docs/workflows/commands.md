...
| **`/dev-deep <requirement> [--auto] [--retry=N] [--budget=N] [--resume]`** | Dev Flow | **Deep-Dev Mine-First Deep-Task Track**: for requirements too vague to write acceptance criteria (code OR analysis) — 6-part requirement mining → gated task-tree planning → node-wise execution with fixed retry/replan brakes → delivery with audit trail & recovery card; `--resume` continues from `.ocp/dev-deep-state.md` (see [Deep-Task Protocol](dev-deep.md)). Standalone protocol, NOT a `/dev` preset |
