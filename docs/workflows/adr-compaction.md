...
This is a context-efficiency aid, **not a security sandbox**. The verified native cancellation path is OpenCode 1.18.15 `read`. Adapters also recognize `read_file`, `grep`, and a finite set of obvious `bash`/`shell` shapes. Arbitrary scripts, shell obfuscation, Git history, MCP, unintegrated tools and cumulative native-reader budgets are **not universally enforced**. Do not route around a block; use targeted `adr_context` history retrieval.
