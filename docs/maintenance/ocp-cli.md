...
- **opencode is pinned to major v2** (the OCP 2.x line) — OCP's plugins (`@opencode/plugin` SDK) and V2-native config contract target the opencode v2 API surface, so the opencode installer (`install/scripts/tools/opencode.sh|.ps1`, wired as `@script:opencode`) resolves the *newest v2* release — never "latest overall". A v1 binary inside your user profile is **upgraded in place** to the newest v2 (the documented runtime transition; v2 keeps the same config locations). An opencode owned by another manager (brew/choco, outside the profile) is refused as externally managed, and a v3+ binary is never downgraded without consent. The same both-direction runtime/package gate runs in the `install/install.sh|.ps1` bootstrap and in `provisionTools`: a v1 runtime with the v2 package (or vice versa) is refused before anything is touched.
- **Fresh `ocp install` is locked too** — applying a repo copy whose major differs from the installed one is refused before anything is touched, in *both* directions. The upgrade path from OCP 0.x (v1 runtime) to OCP 2.0.0 (v2 runtime) is a deliberate major jump: `ocp init` + the quick-install one-liner below, or a fresh install — the installer then pins the v2 runtime for you.
- To actually jump a major, reinstall fresh from the **latest release**: `ocp init` backs up + clears the target (removing `installed.version`), then run the README quick-install one-liner — `curl -fsSL https://raw.githubusercontent.com/kenlin8827/opencode-prime/main/install.sh | bash` (POSIX) or `irm https://raw.githubusercontent.com/kenlin8827/opencode-prime/main/install.ps1 | iex` (Windows). It downloads the newest release directly. `ocp upgrade` / `ocp install` alone cannot do the jump — they refuse a mismatched-major apply (a `git pull` that advances the repo copy past the installed major is refused the same way).
