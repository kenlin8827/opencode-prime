...
- **opencode is pinned to major v1** — OCP's plugins and `@opencode-ai/*` dependencies target the v1 API surface, so the opencode installer (`install/scripts/tools/opencode.sh|.ps1`, wired as `@script:opencode`) resolves the *newest v1* release — never "latest overall" — and refuses when a v2+ binary is on PATH. A detected v2 is never overwritten, shadowed, or "upgraded over"; uninstall it, install the newest v1, then re-run. The same refusal runs in the `install/install.sh|.ps1` bootstrap and in `provisionTools` (which additionally skips opencode-gated post-install steps while v2 is present).
- **Fresh `ocp install` is locked too** — applying a repo copy whose major differs from the installed one is refused before anything is touched, in *both* directions (installed v2 + repo v1 rolls back nothing, and the symmetric jump forward is no safer).
