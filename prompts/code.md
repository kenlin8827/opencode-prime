...
3. **Locate** — index first: when `CodeGraph`/`GitNexus`/`Serena` are `ready` in the `[PROJECT CAPABILITIES]` block, one graph/symbol query replaces a grep-read loop. For broad text/regex, use the built-in `tgrep_search` tool (LLM tool, not a bash binary) if it reports any of the 5 indexed states (`ready`, `no-watcher`, `stale`, `building`, `no-index`); otherwise (`tgrep=no-cli` or `tgrep=unavailable`) use native `grep` / `glob` / `bash rg`. `freshness` is **required** — pick from `tgrep=<state>` in the block per the rules below:
