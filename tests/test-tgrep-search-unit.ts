...
assert(tgrepOptionsFrom(root, '{}').enabled === true, "omitted tools.tgrep defaults ON (installer toolEnabled parity)")
assert(parseTgrepOptions(root, undefined).enabled === true, "undefined switch value defaults ON")
assert(tgrepOptionsFrom(root, '{"tools":{"tgrep":{"enabled":false}}}').enabled === false, "explicit enabled:false disables integration")
